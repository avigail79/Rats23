...
def getSlopePC(result, pCorrect, unscaled=False):
    ...
    if unscaled:
        ...
        assert ((pCorrect > theta0[3]) & (
                    pCorrect < (1 - theta0[2]))), 'pCorrect must lay btw {:.2f} and {:.2f}'.format(theta0[3],
                                                                                                   (1 - theta0[2]))
    ...
    if sigName in ['norm', 'gauss', 'neg_norm', 'neg_gauss']:
        ...
        stimLevel = theta0[0] - theta0[1] * (np.log(1 / pCorrectUnscaled - 1) - np.log(1 / PC - 1)) / 2 / np.log(
            1 / alpha - 1)
        C = 2 * np.log(1 / alpha - 1) / theta0[1]
        d = np.log(1 / PC - 1)
        slope = C * np.exp(-C * (stimLevel - theta0[0]) + d) / (1 + np.exp(-C * (stimLevel - theta0[0]) + d)) ** 2
        ...
        C = np.log(-np.log(alpha)) - np.log(-np.log(1 - alpha))
        stimLevel = np.exp(
            theta0[0] + theta0[1] / C * (np.log(-np.log(1 - pCorrectUnscaled)) - np.log(-np.log(1 - PC))))
